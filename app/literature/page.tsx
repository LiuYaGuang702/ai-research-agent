        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-6">
            ...
            {round > 0 && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-[#161b2b]/80 border border-white/[0.06] p-5">
                ...
                <div className="flex items-center gap-2 mt-4"><span className="text-gray-600 text-xs">迭代轮次:</span>{[1,2,3,4,5].map(r => (<div key={r} className={"w-6 h-6 rounded-lg flex items-center justify-center text-xs font-medium transition-all "+(r <= round ? "bg-teal-500/20 text-teal-400 border border-teal-500/30" : "bg-white/[0.04] text-gray-600 border border-white/[0.06]")}>{r}</div>))}</div>
              </motion.div>
            )}
          </div>
          <div className="lg:col-span-2">
            {round === 0 ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2 mb-2"><div className="flex items-center gap-4 text-sm"><span className="text-gray-400">共检索 <span className="text-white font-semibold">{round < 5 ? "..." : 6}</span> 篇相关文献</span><span className="text-gray-600">|</span><span className="text-gray-400">已选 <span className="text-cyan-400 font-semibold">{sel.length}</span> 篇</span>{round < 5 && <span className="flex items-center gap-1 text-cyan-400"><svg className="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>筛选中</span>}</div></div>
                <AnimatePresence>
                    ...
                    <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.05 }} onClick={() => toggle(p.id)} className={"rounded-2xl border p-5 cursor-pointer transition-all " + (sel.includes(p.id) ? "bg-teal-500/10 border-teal-500/30" : "bg-[#161b2b]/60 border-white/[0.06] hover:border-white/[0.12]")}>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-4"><h3 className="text-white font-medium leading-snug">{p.title}</h3><div className="flex items-center gap-2"><span className="flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] bg-white/[0.04] text-gray-500">R{p.round}</span><span className="flex-shrink-0 px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-500/15 text-teal-400">{(p.relevance*100).toFixed(0)}%</span></div></div>
                ...
                {round >= 5 && (
                  <div className="flex items-center gap-3 pt-2 justify-end"><button className="px-5 py-2.5 text-gray-400 bg-white/[0.04] border border-white/[0.08] rounded-xl text-sm hover:bg-white/[0.08] transition-all">重新筛选</button><button className="px-5 py-2.5 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-xl text-sm font-medium hover:shadow-lg transition-all">进入信息抽取 →</button></div>
                )}
              </div>
            )}
          </div>
